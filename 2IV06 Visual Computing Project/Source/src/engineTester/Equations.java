...
public class Equations {
	...
	public static void CalculatePredictedPositions(Hair hair, Vector3f force, float deltaT) {
		...
		for (int i = 1; i < hair.getParticles().size(); i++) {
			...
			particle.setPredictedPosition(new Vector3f(particle.getPosition().x + deltaT * particle.getVelocity().x * time_damping + deltaT * deltaT * force.x, particle.getPosition().y + deltaT * particle.getVelocity().y * time_damping + deltaT * deltaT * force.y, particle.getPosition().z + deltaT * particle.getVelocity().z * time_damping + deltaT * deltaT * force.z));
		}
	}
	...
	public static void FixedDistanceContraint(Hair hair) {
		for (int i = 1; i < hair.getParticles().size(); i++) {
			...
			particle.setPredictedPosition(new Vector3f(x1.x + delta.x * particleDistance, x1.y + delta.y * particleDistance, x1.z + delta.z * particleDistance));
